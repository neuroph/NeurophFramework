...
public class GRUStockPricePredictionExample {
    ...
    private static void trainNetwork() {
        ...
        RNN gru = new GRU(inputsCount, hiddenCount, new MatrixInitializer(MatrixInitializer.Type.Uniform, 0.1, 0, 0));
        BackPropagationThroughTime bptt = new GRUBackPropagationThroughTime();
        bptt.setLearningRate(learningRate);
        gru.setLearningRule(bptt);

        System.out.println("Training network...");
        bptt.learn(trainingSet, maxIterations);
        System.out.println("Training completed.");

        testNetwork(gru, testSet);
    }

    private static void testNetwork(RNN gru, DataSet testSet) {
        ...
        for (int j = 0; j < sequence.size(); j++) {
            ...
            for (int timestep = 0; timestep < seq.length() - 1; timestep++) {
                ...
                gru.activate(timestep, valuesInTimesteps);

                DoubleMatrix predictedResult = gru.decode(valuesInTimesteps.get("output" + timestep));
            ...
            BackPropagationThroughTime bptt = (BackPropagationThroughTime) gru.getLearningRule();
